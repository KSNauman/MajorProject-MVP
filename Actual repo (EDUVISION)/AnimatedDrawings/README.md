# Animated Drawings — MVP Fork 🎨

> **Our custom fork** of [facebookresearch/AnimatedDrawings](https://github.com/facebookresearch/AnimatedDrawings).
> We've added a Gemini Vision pipeline so you can animate your own drawings with **zero Docker setup**.

---

## 🤝 Setup for Collaborators

Hey! Welcome to the project. Here's how to get running in ~5 minutes:

### 1. Clone & Install
```bash
git clone https://github.com/KSNauman/Animated-Drawings-MVP-.git
cd Animated-Drawings-MVP-

# Create the conda environment (Python 3.8 required by AnimatedDrawings)
...
We use the **Google Gemini API** (free!) for AI joint detection on drawings.
...
> The `.env` file is git-ignored — your key stays local, never gets committed.

### 3. Animate a Drawing!
```bash
conda activate animated_drawings

# Drop any drawing image in the project folder, then:
python run.py your_drawing.png dab

# Available motions: dab, wave_hello, zombie, jumping_jacks, jesse_dance, jumping
```

Your animated GIF will appear as `animated_your_drawing.gif` 🎉

### 4. What's in This Repo

| File | What it does |
|------|-------------|
| `run.py` | Main entry point — image → animated GIF |
| `image_to_annotations_claude.py` | Gemini Vision joint detection (no Docker!) |
| `run_my_drawing.py` | Manual/proportional joint placement (no API key needed) |
| `animate.py` | Renders annotation files into a GIF |
| `.env.example` | Template for your API key setup |

### 5. Troubleshooting

- **Rate limited (429)?** The free Gemini tier allows ~15 requests/minute. The script auto-retries, just wait a moment.
- **Joints look wrong?** Use `run_my_drawing.py --mode manual` and visually fix with `examples/fix_annotations.py`.
- **Module not found?** Make sure you're in the `animated_drawings` conda env: `conda activate animated_drawings`.
