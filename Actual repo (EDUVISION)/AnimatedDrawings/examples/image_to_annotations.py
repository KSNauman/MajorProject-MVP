...
def segment(img: np.ndarray):
    # 1. AI Background Removal for semantic isolation
    ai_mask = rembg.remove(img, only_mask=True)

    # 2. Classic OpenCV Adaptive Thresholding for crisp pen lines
    ...
    # 4. Hybrid Intersection: Keep only thresholded pen strokes that fall inside the AI silhouette
    combined = cv2.bitwise_and(thresh, ai_mask)

    # 5. Floodfill to remove stray disconnected artifacts
    ...
    if mask is None:
        # Fallback to raw AI mask if contour math fails
        logging.warning("OpenCV contour logic failed, falling back to pure rembg mask.")
