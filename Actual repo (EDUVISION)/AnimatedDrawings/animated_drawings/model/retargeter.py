...
class Retargeter():
    ...
    def _determine_projection_plane_normal(self, group_name: str, joint_names: List[str], projection_method: str) -> npt.NDArray[np.float32]:
        ...
        for joint_name in joint_names:
            try:
                joints_idxs.append(self.bvh_joint_names.index(joint_name))
            except ValueError:
                raise ValueError(
                    f"\n[Error] Joint '{joint_name}' was not found in the BVH motion file. "
                    f"This usually means you are using a mismatching retarget configuration.\n"
                    f"Please make sure to match the motion file with its correct retarget config:\n"
