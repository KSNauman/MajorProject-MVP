...
class ARAP():
    ...
    def _xy_to_barycentric_coords(self,
        ...
        for p_xy in points:
            ...
            # point is outside mesh. Find the nearest vertex on the mesh and snap to it.
            if not len(containing_t_idxs):
                # Calculate distance to all mesh vertices
                dists = np.linalg.norm(vertices - p_xy, axis=1)
                v_nearest_idx = np.argmin(dists)
                
                # Find the first triangle containing this vertex
                containing_t_idxs = [i for i, t in enumerate(triangles) if v_nearest_idx in t]
                if len(containing_t_idxs) > 0:
                    t_idx = containing_t_idxs[0]
                    # Snapped successfully! Construct custom barycentric coordinates for this vertex
                    vertex_ids = triangles[t_idx]
                    uvw = np.array([1.0 if vid == v_nearest_idx else 0.0 for vid in vertex_ids], dtype=np.float32)
                    b_coords.append(list(zip(vertex_ids, uvw)))
                    pin_mask.append(True)
                    
                    msg = f"point {p_xy} was outside mesh; automatically snapped to nearest vertex ID {v_nearest_idx}."
                    print(msg)
                    logging.info(msg)
                    continue
                else:
                    # Fallback to skipping if no triangle contains this vertex (should not happen)
