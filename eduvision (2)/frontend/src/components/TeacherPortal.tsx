          ...
          <div className="bg-white/90 rounded-3xl p-5 border border-slate-100 shadow-sm flex flex-col h-full overflow-y-auto scrollbar-thin">
            ...
            <div className="flex flex-col md:flex-row items-start justify-between gap-4 pb-4 border-b border-slate-100">
              <div className="text-left">
                ...
                />
                <p className="text-[11px] text-slate-404 mt-0.5 font-mono">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                    selectedStory.reviewStatus === "rejected"
                      ? "bg-rose-550 bg-rose-500 text-white shadow-sm"
                      : "bg-slate-100 hover:bg-slate-200 text-slate-600"
                  }`}
                >
                  ...
                  {ch.checkpoint && (
                    <div className="mt-2 pt-2 border-t border-slate-100 pl-2 bg-rose-50/30 rounded-xl max-w-lg">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                        {ch.checkpoint.correctOption && (
                          <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold border bg-emerald-50 border-emerald-200 text-emerald-700">
                            {ch.checkpoint.correctOption} ✓
                          </span>
                        )}
                        {ch.checkpoint.distractorOptions?.map((opt, oIdx) => (
                          <span
                            key={oIdx}
                            className="px-2 py-0.5 rounded-lg text-[10px] font-bold border bg-white border-slate-200 text-slate-500"
                          >
                            {opt}
                          </span>
                        ))}
